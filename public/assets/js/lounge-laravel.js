...
async function searchProducts(searchTerm) {
    try {
        const response = await fetch(`/app/lounge/products/search?q=${encodeURIComponent(searchTerm)}`);
        if (response.ok) {
...
async function filterProductsByCategory(categoryId) {
    try {
        let url = '/app/lounge/products/search';
        if (categoryId) {
...
async function addToCart(productId, quantity = 1) {
    try {
        const response = await fetch('/app/lounge/cart/add', {
            method: 'POST',
            headers: {
...
async function removeFromCart(productId) {
    try {
        const response = await fetch(`/app/lounge/cart/remove/${productId}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
...
async function updateQuantity(productId, change) {
    try {
        const response = await fetch('/app/lounge/cart/update', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
async function clearCart() {
    ...
    try {
        const response = await fetch('/app/lounge/cart/clear', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
...
async function searchCustomers() {
    ...
    try {
        const response = await fetch(`/app/customers/search/query?search=${encodeURIComponent(searchTerm)}`);
        const data = await response.json();
...
async function saveQuickCustomer() {
    ...
    try {
        const response = await fetch('/app/customers/quick/create', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
async function viewLowStock() {
    ...
    try {
        const response = await fetch('/app/products/low-stock/list');
        const data = await response.json();
        ...
        if (data.success && data.products.length > 0) {
            tbody.innerHTML = data.products.map(product => `
                <tr>
                    ...
                    <td>
                        <a href="/app/products/${product.id}/edit" class="btn btn-sm btn-primary" target="_blank">
...
async function viewDailySales() {
    ...
    try {
        const response = await fetch('/app/lounge/daily-report');
        const data = await response.json();
...
async function completePayment() {
    ...
    try {
        ...
        const response = await fetch('/app/lounge/checkout', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function printReceipt(saleId) {
    window.open(`/app/sales/${saleId}/receipt`, '_blank');
}
